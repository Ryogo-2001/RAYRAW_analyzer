...
class UserParasite : public VEvent
{
private:
  RawData*      rawData;
...
UserParasite::ClassName()
{
  static TString s_name("UserParasite");
  return s_name;
}

//_____________________________________________________________________________
UserParasite::UserParasite()
  : VEvent(),
    rawData(new RawData)
{
}
...
UserParasite::ProcessingNormal()
{
  ...
  {
    Int_t kvc_nhits = 0;
    const HodoRHitContainer& cont = rawData->GetE72KVCRawHC();
    Int_t nh = cont.size();
    Int_t nh1 = 0, nh2 = 0;
    for(Int_t i=0; i<nh; ++i){
      HodoRawHit *hit = cont[i];
  ...
  ///// KVCSUM
  {
    ...
    const HodoRHitContainer& cont = rawData->GetE72KVCSUMRawHC();
    Int_t nh = cont.size();
    for(Int_t i=0; i<nh; ++i){
      HodoRawHit *hit = cont[i];
      Int_t seg = hit->SegmentId()+1;
